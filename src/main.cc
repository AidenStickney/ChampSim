...
int main(int argc, char** argv) // NOLINT(bugprone-exception-escape)
{
  ...
  long long estimated_instructions = 0;
  long long sim_points = 0;
  ...
  auto estimated_instr_option = app.add_option("-e,--estimated-instructions", estimated_instructions,
                                               "The estimated number of instructions in the detailed phase. If not specified, run to the end of the trace.");
  auto* sim_points_option = app.add_option("-p,--sim-points", sim_points, "The number of simulation points in the detailed phase. If not specified, no simulation points are used.");
  ...
  const bool estimated_given = estimated_instr_option->count() > 0;
  const bool sim_points_given = sim_points_option->count() > 0;
  ...
  if (sim_points_given && estimated_given) {
    // Detailed simulation phase with simulation points
    long long subset_instr = estimated_instructions / sim_points;

    for (std::size_t i = 0; i < sim_points; ++i) {
      phases.push_back(champsim::phase_info{
          "Simulation" + std::to_string(i),
          false,
          subset_instr,
          std::vector<std::size_t>(std::size(trace_names), 0),
          trace_names});
    }
  } else {
    // Standard simulation phase
    phases.push_back(champsim::phase_info{
        "Simulation",
        false,
